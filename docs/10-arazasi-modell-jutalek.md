...
- Globális jutalék-statisztika **(J8c, SLO-123)**: a superadmin vezérlőpult (`Super/Dashboard`) valódi tartalma — havi jutalékbevétel (MRR-proxy: a hónapra elhatárolt nettó jutalék, `tenant_billing_periods` aggregátumból, §5.4), top tenantok forgalom szerint, plafont elérők száma, küszöb alatt „ragadt" (van forgalma, de még nem jutalékköteles) tenantok aránya és aktiválási tölcsér (aktív → forgalmazó → fizető), lejárt számlák / felfüggesztés-kockázat (`due_at` + `DunningSweep::GRACE_DAYS`).
  ...
  - **Bruttó vs. nettó bevétel (SLO-127):** az MRR-proxy a **kiszámlázható nettó**, nem a bruttó elhatárolás. Period-onként `max(0, commission_minor + correction_minor)`, a `void` időszakok kihagyásával — a `commission_accrued_minor` (bruttó) és a `correction_total_minor` (jóváírás, ≤ 0) külön mezőként marad látható. Két szándékos következmény: (a) a hónap saját jutalékát meghaladó jóváírás **nem** negatív bevétel, mert a maradék a következő periodba megy át (§6.5 carry-over) és ott számolódik újra — a dupla levonás így kizárt; (b) a `void` period kimarad, mert vagy nem is volt számlája, vagy stornóztuk. Ezért `nettó ≠ bruttó + jóváírás`, viszont a nettó **pontosan** az életciklus-bontás (nyitott/kiszámlázva/befizetve/lejárt) összege. A „fizető tenant" számláló ugyanezt a nettót nézi: a teljesen jóváírt hónap nem fizető hónap (a tölcsérben „ragadt"-ként jelenik meg). A top-tenant tábla is a nettót mutatja, a jóváírás összegével a sor alatt.
