...
3. **Assetek.** `public/brand/`: `logo-icon.svg`, `wordmark.svg`, `sloth-laptop.svg`, `sloth-wave.svg`, `favicon.svg`, `app-icon-512.png`. Vektor a kabalából kötelező (Illustrator Image Trace / Recraft / Vectorizer.ai). ⚠️ **A `hero`: PNG/WebP rétegek, lásd docs/23** — a repülő pózt nem vektorizáljuk, és a fájlok a `resources/images/sloth/` alatt élnek (a `public/` prodon az apex domainen nem szolgálódik ki, SLO-233).
