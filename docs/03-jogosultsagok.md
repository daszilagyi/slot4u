...
Két alak, mert két különböző tény: **létrehozáskor a `staff_id` kötelező és sajátnak kell lennie**, mozgatáskor opcionális (a hiánya = „marad a mostani erőforrás", és azt a foglalást a controller már ownership-ellenőrizte). A létrehozás szigorúbb foka nem formalitás: `staff_id` nélkül (helyiség-bérlés vagy `no_time_slot` mód) a korlátozott aktor olyan foglalást hozna létre, amely a `BookingVisibility::owns()` szerint **azonnal kiesik a saját listájából** — láthatatlan, tehát nem is mondható le és nem kereshető vissza. A **mozgatás** ugyanezért tilos idegen naptárba: a foglalás a mentés pillanatában eltűnne a mozgatója elől.
