...
- **Guardrailek** (mindegyikre Pest teszt):
  ...
  - **A seedelt fixture-felhasználó elfogadja az érvényben lévő jogi dokumentumokat** (SLO-209). Enélkül minden demo-fiók a „nulladik napon" áll, és az `EnsureLegalConsent` (SLO-161) a re-elfogadó falon tartja: a landingről érkező látogató a megígért dashboard helyett egy 0.1-draft dokumentumokat felsoroló űrlapot kap. ⚠️ **És nem csak az admin nézetben:** a kapu minden bejelentkezett kérésre lefut, így amíg a session él, a tenant **publikus** foglalóoldala is a falra irányít — vagyis a landing iframe-je is. A `DemoPersona::createStaffUser()` ezért a szerepek beállítása után `LegalDocumentRegistry::outstandingFor()` + `RecordConsent` párossal rögzíti az elfogadást, `ConsentContext::Reconsent` kontextussal (nem tett senki pipát űrlapra — ezt a képernyőt helyettesíti). ⚠️ **A javítás a seedben van, nem a middleware-ben:** az `is_demo` kihagyása a kapuból egy valódi jogi kontrollt gyengítene egy sales-demo kedvéért. A fixture-user elfogadása ugyanúgy fikció, mint a foglalásai, és a sorok `tenant_id`-ja demo tenantra mutat, tehát megkülönböztethető a valódi bizonyítéktól. Új dokumentum-verzió közzététele után a demo-fiókok újra elfogadatlanok — amíg a hajnali `demo:reset` (SLO-191) újra nem építi őket; legfeljebb egy nap, és magától rendeződik.

  - **Publikus demo-belépőpont** (SLO-192, `docs/21` §2.1): a `demo/login` útvonal aláírt, 15 perces, IP-nként percenként 10-szer hívható, és **kizárólag `is_demo = true` tenanton él** — másra 404 (nem 403: a téves tipp ne igazolja vissza, hogy a tenant létezik). A landing persona-listája ugyanebből a jelzőből épül, nem kódba írt névsorból. A demo tenant az egyetlen, amelynek oldalait `iframe`-be lehet tenni (`SecurityHeaders`: `X-Frame-Options` elhagyva + `frame-ancestors` a központi domainre); minden más tenant `DENY`-t kap. ⚠️ **Ehhez a központi domain oldalán `frame-src` is kell** a demo originekre (SLO-213): a `frame-ancestors` csak azt mondja ki, hogy a demo *hagyja magát* beágyazni — hogy a marketing oldal *beágyazhat-e*, azt a saját policy-ja dönti el, és `frame-src` nélkül a `default-src 'self'` tiltja. Mindkét felet ugyanaz a middleware állítja be, kérésenként. A demo minden oldalán — publikuson és adminon is — ott a „DEMO · fiktív adatok" sáv.
...
- Jogi kapu (SLO-209): `demo:seed` után egyetlen demo-felhasználónak sincs elfogadatlan dokumentuma; az aláírt demo-belépő után a **dashboard** és a tenant **publikus** oldala is nyílik; és — ellensúlyként — egy valódi tenant staff-userét a fal **továbbra is** megállítja.
...
6. **Persona: Fényliget Rendezvényház** — quote flow minden státuszban, üzenetváltással.
