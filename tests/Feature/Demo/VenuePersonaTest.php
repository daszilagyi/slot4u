...
/*
|--------------------------------------------------------------------------
| „Pelso Rendezvényház" (SLO-186, docs/20 §2.4)
|--------------------------------------------------------------------------
|
 ...
 */
function pelsoQuotes(Tenant $tenant)
{
...
it('shows the whole quote pipeline at once, with real money behind the won ones', function () {
    ...
    foreach (QuoteRequestStatus::cases() as $status) {
        expect(pelsoQuotes($tenant)->where('status', $status->value)->count())
            ->toBeGreaterThan(0, "No quote request left in status {$status->value}");
    }

    $accepted = pelsoQuotes($tenant)->where('status', QuoteRequestStatus::Accepted->value)->get();
    ...
    // is a dead end.
    $quoted = pelsoQuotes($tenant)->where('status', QuoteRequestStatus::Quoted->value)->first();
    ...
    $messages = DB::table('quote_request_messages')
        ->whereIn('quote_request_id', pelsoQuotes($tenant)->pluck('id'))
        ->get();
    ...
    $withParams = pelsoQuotes($tenant)->whereNotNull('parameters')->first();
...
it('takes an enquiry from the public page and holds an unapproved room let', function () {
    ...
    $this->get(tenantHost($slug, '/'))
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page->where('profile.name', 'Pelso Rendezvényház'));
    ...
    $before = pelsoQuotes($tenant)->count();
    ...
    expect(pelsoQuotes($tenant)->count())->toBe($before + 1);
