        ...
        <AppLayout>
            ...
            <div className="flex w-full max-w-3xl flex-col gap-6">
                ...
                <section className="rounded-xl border border-border p-6">
                    <h2 className="mb-4 text-lg font-medium">{t('super.tenants.show.edit_title')}</h2>
                    <form onSubmit={submit} className="flex flex-col gap-4">
                        <div className="flex flex-col gap-2">
                            <Label htmlFor="name">{t('super.tenants.field.name')}</Label>
                            <Input id="name" value={form.data.name} onChange={(e) => form.setData('name', e.target.value)} />
                            {form.errors.name ? <p className="text-sm text-red-500">{form.errors.name}</p> : null}
                        </div>
                        <div className="flex flex-col gap-2">
                            <Label htmlFor="slug">{t('super.tenants.field.slug')}</Label>
                            <Input id="slug" value={form.data.slug} onChange={(e) => form.setData('slug', e.target.value)} />
                            {form.errors.slug ? <p className="text-sm text-red-500">{form.errors.slug}</p> : null}
                        </div>
                        <div className="flex flex-col gap-2">
                            <Label htmlFor="timezone">{t('super.tenants.field.timezone')}</Label>
                            <Input id="timezone" value={form.data.timezone} onChange={(e) => form.setData('timezone', e.target.value)} />
                            {form.errors.timezone ? <p className="text-sm text-red-500">{form.errors.timezone}</p> : null}
                        </div>
                        <div className="flex flex-col gap-2">
                            <Label htmlFor="locale">{t('super.tenants.field.locale')}</Label>
                            <select
                                id="locale"
                        ...
                        <Button type="submit" className="self-start" disabled={form.processing}>
                ...
                {/* Status actions */}
                <section className="rounded-xl border border-border p-6">
                    <h2 className="mb-1 text-lg font-medium">{t('super.tenants.show.actions_title')}</h2>
                    <p className="mb-4 text-sm text-muted-foreground">
                        {t('super.tenants.col.trial_ends')}:{' '}
                        {tenant.trial_ends_at ? formatDate(tenant.trial_ends_at) : t('super.tenants.show.no_trial')}
                    </p>
                    <div className="flex flex-wrap gap-2">
                        {tenant.status === 'suspended' || tenant.archived ? (
                            <Button variant="outline" onClick={() => post('activate')}>
                                {t('super.tenants.action.activate')}
                            </Button>
                        ) : (
                            <Button variant="outline" onClick={() => post('suspend')}>
                                {t('super.tenants.action.suspend')}
                            </Button>
                        )}
                        <Button variant="outline" onClick={() => post('extend-trial')}>
                ...
                {/* Feature toggles */}
                <section className="rounded-xl border border-border p-6">
                    <h2 className="mb-4 text-lg font-medium">{t('super.tenants.show.features_title')}</h2>
                    <div className="flex flex-col divide-y divide-border">
                        {featureStates.map((feature) => (
                            <label key={feature.code} className="flex items-center justify-between py-2.5 text-sm">
